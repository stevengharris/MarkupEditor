...
class InsertTable: MarkupDelegate {
    ...
    /// any way around it.
    func setActions() {
        Self.tests[0].action = { self.webView.insertTable(rows: 2, cols: 2) }
        Self.tests[1].action = { self.webView.insertTable(rows: 2, cols: 2) }
        Self.tests[2].action = { self.webView.insertTable(rows: 2, cols: 2) }
    }
