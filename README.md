...
The MacOS and Mac Catalyst toolbars have the same contents and functionality. They look a bit different because the former is rendered as part of the MarkupWKWebView as part of [markupeditor-base project](https://github.com/stevengharris/markupeditor-base), while the latter is built and displayed using SwiftUI. The icons for all the main toolbar use SF-Symbols, so they are consistent with Mac and iOS users' expectations. For MacOS, this is accomplished by customizing Resources/toolbarconfig.json (which originates from markupeditor-base using Material Design icons). On MacOS, the "Show HTML" item is in the View menu, and the File menu contains the New, Open, Save, and Save As items.
