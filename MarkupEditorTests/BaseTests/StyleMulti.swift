...
class StyleMulti: MarkupDelegate {
    ...
    func setActions() {
        ...
        Self.tests[1].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .H1) }
        Self.tests[2].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .H1) }
        Self.tests[3].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .H1) }
    }
