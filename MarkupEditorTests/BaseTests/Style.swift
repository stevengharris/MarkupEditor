...
class Style: MarkupDelegate {
    ...
    /// any way around it.
    func setActions() {
        Self.tests[0].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .H1) }
        Self.tests[1].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .H6) }
        Self.tests[2].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .P) }
        Self.tests[3].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .PRE) }
        Self.tests[4].action = { let state = await self.webView.getSelectionState(); await self.webView.replaceStyle(state.style, with: .PRE) }
    }
