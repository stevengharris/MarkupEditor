...
class TableActions: MarkupDelegate {
    ...
    /// any way around it.
    func setActions() {
        Self.tests[0].action = { await self.webView.deleteRow() }
        Self.tests[1].action = { await self.webView.deleteCol() }
        Self.tests[2].action = { await self.webView.deleteTable() }
        Self.tests[3].action = { await self.webView.addRow(.before) }
        Self.tests[4].action = { await self.webView.addRow(.after) }
        Self.tests[5].action = { await self.webView.addCol(.before) }
        Self.tests[6].action = { await self.webView.addCol(.after) }
        Self.tests[7].action = { await self.webView.addHeader() }
        Self.tests[8].action = { await self.webView.borderTable(.cell) }
        Self.tests[9].action = { await self.webView.borderTable(.header) }
        Self.tests[10].action = { await self.webView.borderTable(.outer) }
        Self.tests[11].action = { await self.webView.borderTable(.none) }
    }
